...
if __name__ == "__main__":    
	...
	np.random.seed(4294967295) #reserve integer-valued seeds for pattern generation
    ...
    if learn:
	st=time.time()
	weights_list=correlation_training(neu_paras,feature_data,current_weights, initial_weights, fea_count_means, fea_labels,n_cycles, learning_rate_elig, top_elig_fraction,momentum_factor,seed)
	et=time.time()
	if os.path.exists(outpath+batchname+'cur_weights_list.npy'):
	...
	np.save(outpath+batchname+  'bkgd_rates_stepsize_'+str(stepsize), mean_absent_rates*1000/Tprobe) #convert to Hertz
